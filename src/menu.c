...
{
    if (runningApp != NULL)
    { /* running app is set by menus() not genericMenus() */
        (*runningApp)();
        return;
    }
    ...
    else if (UP_BTN_AND_CONSUME) /* handle slider/soft button clicks */
    {
        ...
        if (G_selectedMenu > G_currMenu)
        {
            ...
            while (((G_selectedMenu->attrib & SKIP_ITEM) || (G_selectedMenu->attrib & HIDDEN_ITEM)) && G_selectedMenu > G_currMenu)
                G_selectedMenu--;
...
void genericMenu(struct menu_t *L_menu, MENU_STYLE style)
{
    ...
    else if (UP_BTN_AND_CONSUME) /* handle slider/soft button clicks */
    {
        ...
        if (L_selectedMenu > L_currMenu)
        {
            ...
            while ((L_selectedMenu->attrib & SKIP_ITEM) && L_selectedMenu > L_currMenu)
                L_selectedMenu--;
...
const unsigned char *splash_word_things[] = {
    "Cognition Module",
    "useless bits",
    "backdoor.sh",
    "exploit inside",
